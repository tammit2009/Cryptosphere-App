...
const getData = async () => {
    const resp = await fetch(`${baseUrl}/api/v1/tvdemo/BTCBUSD/1m`);
...
const renderChart = async () => {
    ...
    const rsi_data = klinedata.filter((d) => d.rsi)
    ...
    const macd_fast_data = klinedata.filter((d) => d.macd_fast)
    ...
    const macd_slow_data = klinedata.filter((d) => d.macd_slow)
    ...
    const macd_histogram_data = klinedata.filter((d) => d.macd_histogram)
                              .map((d) => ({ 
                                  time: d.time, 
